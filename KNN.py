...
class KNN:
	...
	def getNeighbors(self, input):	
		...
		print(distances)
		# 選取K個最相近的sample做為鄰居
		neighbors = distances[:self.k]  #選前3個
		#--------------------
		return neighbors
	#------------------------------
	...
	def vote(self, neighbors):
		...
		for neighbor in neighbors:
			...
			classCounter[neighbor[2]] += 1  # dictionary
			print(classCounter)
		#--------------------
		# 票數最多的Label即為相近的sample
		# 輸出它的Label成為預測結果 classCounter.most_common(1) = [[('Science', 2)]] "(1)"是Get1個
		selected = classCounter.most_common(1)[0][0]
		nums = classCounter.most_common(1)[0][1]
	...
	def predict(self, inputs):		
		predictResults = list()
		#--------------------
		for input in inputs: #  (79,77,50,65) 1次輸進去
			# 統計鄰居
			neighbors = self.getNeighbors(input)
			# 投票
			result,nums = self.vote(neighbors)
			# 紀錄當選者(預測結果)
			predictResults.append(result)
			predictResults.append(nums)
		#--------------------
...
if __name__ == '__main__':
	...
	print(predictResults[0])
	print(predictResults[1])